...
void SimpleWindow::on_pushEqual_clicked() {
  ...
  if (ui->ownX2->text() != "" && ui->output->text().contains("x")) {
    ...
    str[i] = '\0';
    c.calcExpressionWithX(str, &result, ui->ownX2->text().toDouble());
...
void SimpleWindow::on_pushGraph_clicked() {
  ...
  if (ui->output->text() != "" && ui->ownX2->text() == "") {
    ...
    correct = c.calcExpressionForGraphWithoutX(str, array_Y);
    ...
    correct = c.calcExpressionForGraphWithX(str, array_Y,
