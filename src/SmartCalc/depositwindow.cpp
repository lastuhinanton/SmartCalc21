  ...
  if (ui->deposit->text() != "" && ui->period->text() != "" &&
    ...
    if (isValidatedDeposit(deposit_check) && isValidatedDeposit(period_check) &&
      ...
      if (ui->capitalisationCheck->isChecked()) {
        ...
        profit_sum = deposit;
        for (int i = 0; i <= period % days_of_month; i++) {
          profit_sum += ((profit_sum * rate * days_of_month) / 365) / 100;
        }
        profit_percent = profit_sum - deposit;
      } else {
        profit_percent = ((deposit * rate * period) / 365) / 100;
        profit_sum = deposit + profit_percent;
      }
      tax_sum = (profit_sum * tax) / 100;
