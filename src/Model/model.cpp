...
namespace s21 {
    ...
    int Model::encode(char *s) {
      ...
      while (i < OPERATORS_COUNT && code == -1) {
        if (!std::strcmp(s, operator_str[i])) {
          code = operator_int[i];
        }
        i++;
      }
    ...
    int Model::queue_input(queue *queue, const std::string &str) {
    ...
    int Model::getValueFromExpression(const std::string &str, double *answer) {
      int correct = 1;
      std::string str_new = str;
      str_new.erase(std::remove_if(str_new.begin(), str_new.end(),
        [](char c) { return std::isspace(c); }), str_new.end());
